...
- 2026-08-02: The closed-bar dispatch engine now serializes bounded event windows, routes recovered bars through the same atomic cycle port, and persists per-bar recovery progress with the cursor so a mid-gap crash resumes at the first uncommitted sequence. Checkpoints can transfer to an expired-lease takeover owner without discarding history. REST-recovered bars accept an explicit historical decision cutoff, preventing current books/references from leaking into past decisions. The production command/context assembler and lifecycle supervisor remain pending.
